...
@router.get("/get_user/", status_code=status.HTTP_200_OK, response_model=dict)
def get_user(current_user = Depends(oauth2.get_current_supabase_user), db: Session = Depends(get_db)):
    email = current_user.user.email

    user_query = db.query(models.User).filter_by(email=email)
